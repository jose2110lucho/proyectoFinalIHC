...
class InscripcionTable extends Component
{
    public $materias = [];
    public $horarios = [];
    public $materiaCheck = [];
    public $contador= 0;
    /* public $materiasElegidas = []; */
    ...
    public function onChangeCheck($sigla, $nombre, $grupo, $cupos_libres, $docente, $horarioInicio, $horarioFin, $aula, $dias)
    {
        ...
        if ($this->materiaCheck[$sigla]) {
            ...
            $existeCruceMaterias = $this->cruceDematerias($horarioInicio,$horarioFin,explode(",", $dias));
            if(!$existeCruceMaterias){
                $materiaHorario = [
    ...
    function cruceDeMaterias($horarioInicio,$horarioFin,$dias){
            ...
            $inicioPeriodoTiempo = '';
            $finPeriodoTiempo = '';
            $cruceDematerias = false;
            while ($hora_actual < $timestamp_fin) {
                $inicioPeriodoTiempo = date("H:i", $hora_actual);
                $hora_actual = strtotime("+30 minutes", $hora_actual);
                $finPeriodoTiempo = date("H:i", $hora_actual);
                /* dd($this->horarios); */
                $horarios = $this->horarios[$inicioPeriodoTiempo . '-' . $finPeriodoTiempo]; 
                
                $bloquesHorario = [];
                $var = [];
                foreach ($horarios as $horario) {
                    
                    $var = array_intersect($dias, $horario['dias']);
                 
                }

                $bloquesHorario[] = count($var) > 0;
                
            }

            $cruceDematerias = in_array(true, $bloquesHorario);

            return $cruceDematerias;
    ...
    {
        $this->materias = [
            ['sigla' => 'INF220', 'nombre' => 'Estructura de datos I', 'grupo' => 'E1', 'cupos_libres' => '12', 'docente' => 'Ing. Augusto Cespedes', 'horarioInicio' => '18:00', 'horarioFin' => '20:00', 'aula' => '236-45', 'dias' => [1, 3, 5]],
            ...
            ['sigla' => 'INF513', 'nombre' => 'Tecnologia Web', 'grupo' => 'SA', 'cupos_libres' => '10', 'docente' => 'Ing. Evans Balcazar', 'horarioInicio' => '14:00', 'horarioFin' => '15:00', 'aula' => '236-17', 'dias' => [1, 5]],
            //-------------------------------------------------------------------------------------------------
    ...
    {
        return view('livewire.inscripcion-table', ['materias' => $this->materias, 'horarios' => $this->horarios]);
    }
}
